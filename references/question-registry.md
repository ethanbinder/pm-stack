...
| `product-doc-team-name` | product-doc | two-way | One-time team name capture, persisted to `.pm-stack/learnings.md` `Team:` line. Asked only when the user opts into the Roadmap step. Used to title the team-level Roadmap page. | `skip` (no team name persisted; the Roadmap step is silently skipped on this and future runs until set) | yes |
| `product-doc-roadmap` | product-doc | two-way | Per-run opt-in: also create or update `<Team Name> — Roadmap` with this project's status row when on the Confluence path. | `skip` (most runs only want per-tab pages; user can opt in or `tune` later) | yes |
| `product-doc-roadmap-initiative` | product-doc | two-way | When the Roadmap step runs, the project's Initiative cell value — used both as the row identifier and the project's quick title on the roadmap. Changing this value across runs for the same project causes the skill to treat it as a new row. | `<Product Name>` (the project name; safe default that keeps the row identifier stable) | yes |
| `product-doc-roadmap-priority` | product-doc | two-way | When the Roadmap step runs, the project's priority on the team roadmap (Priority column — High / Medium / Low). | `Medium` | yes |
